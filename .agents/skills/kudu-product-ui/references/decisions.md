...
| D50 | Time display | Riyadh 12-hour time | Convert all user-facing timestamps to `Asia/Riyadh` and use `YYYY-MM-DD h:mm am` with lowercase `am`/`pm`; preserve source values for audit needs |
